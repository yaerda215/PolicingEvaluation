...
public class DateUtil {
     ...
     */
    public static String[] getSomeMonths(int x) {
        String[] rqArray = new String[x];
        String needMonth;

        LocalDate today = LocalDate.now();
        LocalDate currentTime = today;
        for (int i = 0; i < x; i++) {
            needMonth = String.valueOf(currentTime.getMonthOfYear());
            if (currentTime.getMonthOfYear() < 10) {
                needMonth = "0" + currentTime.getMonthOfYear();
            }
            rqArray[x - 1 - i] = currentTime.getYear() + "-" + needMonth;
    ...
    public static List<String> getLastWeekDays() {
        ...
        java.time.LocalDate localDate = java.time.LocalDate.now().minusWeeks(1);
        for (int i = 0; i < 7; i++) {
    ...
    public static List<String> getLastMonthDays() {
        List<String> dateList = new ArrayList<>();
        java.time.LocalDate localDate = java.time.LocalDate.now().minusDays(30);
        for (int i = 0; i < 30; i++) {
            localDate = localDate.plusDays(1);
            dateList.add(localDate.format(DateTimeFormatter.ofPattern("yyyy-MM-dd")));
        }
...
//		String[] kk = DateUtilOther.getSomeYears(6);
//		Arrays.stream(kk).forEach(System.out::println);
//	}
